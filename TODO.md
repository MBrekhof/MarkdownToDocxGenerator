...
- [ ] **`MdReportGenenerator.Transform` (file-based) can't save without a template** — `WordManager.New()`/`SaveDoc()` take no path; only `OpenDocFromTemplate(templatePath, outputPath, true)` sets the output path. So the no-template branch calls `New()` then `SaveDoc()` and never writes to `outputPath`. Found during the review; **not yet fixed** (out of the heading/ordering scope). `TransformWithStream` is unaffected. Decide intended behavior before fixing.
